<x-layouts::app :title="__('Edit Role')">
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        <div>
            ...
            <h1 class="mt-1 text-2xl font-semibold text-neutral-900 dark:text-white">Edit {{ $role->name }}</h1>
        </div>

        @include('partials.flash')

        <form method="POST" action="{{ route('roles.update', $role) }}" class="rounded-xl border border-neutral-200 bg-white p-6 dark:border-neutral-700 dark:bg-neutral-900">
            @csrf
            @method('PUT')
